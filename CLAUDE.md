...
| 5a Split view | **written but never built or run** — see docs/stage5a-findings.md, docs/superpowers/specs/2026-09-22-stage-5a-split-view-design.md and docs/superpowers/plans/2026-09-22-stage-5a-split-view.md. Covers R5.1. Chromium's split is graduated and carries no feature flag, so the panes, the divider, the resize and the mini strip were not rebuilt; what Arcium lacked was every way in, because Chrome's entry points live in the tab strip this browser hides. Four were added — a sidebar row dragged onto the page, the row menu, Cmd+Option+S, and a "Split the screen" command in the box — plus one rule about who may share a screen (never two spaces, never a loose page, never a tab already split), an end-the-split-first step before either half leaves its space, and a record in the model so a split survives a quit (the two tabs' TabKeys, the layout and the ratio; model schema 5 to 6 with a no-op migration). No patch was added. Eight commits, 415a563 to da16f52. **The whole stage was written in one pass against headers read in the checkout, with no compile and no test run, at the owner's request while the machine was in use — so nothing in it is verified.** Tests written and not run: 15 for the controller, 9 for the sidebar's adjacency rule, 6 browser tests, plus additions to the box commands, the sidebar view tests (five existing menu expectations updated because the menu gained a row), the command box browser tests, the serialiser and the migration. Five defects were found while writing and fixed before they shipped, each by reading rather than by a test: AddToNewSplit takes one index and CHECKs it (the plan's own sample passed two, which would have crashed on the first split), GetSplitForTab CHECKs its index so row builds ask the tab instead, a returning method cannot be bound to a WeakPtr, the adjacency rule had been written twice, and the controller's constructor was never updated when the last task added its model argument — three parameters against a caller passing four, so it would not have compiled, and it registered with neither the tab strip nor the model, which left the keyboard shortcut and the whole of the persistence work dead (b949f8c). Two panes rather than R5.1's two to four, recorded as deviation D5-1 in section 7 of the master spec. **What is owed: the build, both test binaries, the ten hand rows in docs/stage5a-hand-checks.md, and perf.** R5.2 to R5.4 not started |
