...
3. Test-driven: unit tests in `arcium/test/` for models and services, browser tests for tab-to-space
   ...
   **Never run a full suite without the owner's explicit approval** — neither the whole
   `arcium_unittests` nor the whole `arcium_browsertests`, however quiet the machine looks. The
   unit suite loads every core for minutes on a shared machine, and the browser suite also takes
   over the screen and the pointer. Run only the tests a change touches, by `--gtest_filter`, and
   ask before anything wider. Say so in the report when a full suite was skipped, and which tests
   did run.
