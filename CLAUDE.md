...
3. Test-driven: unit tests in `arcium/test/` for models and services, browser tests for tab-to-space
   ...
   **Never run a full suite until the owner says the computer is free** — neither the whole
   `arcium_unittests` nor the whole `arcium_browsertests`, however quiet the machine looks. That
   message is the approval, and it covers that one sitting: once the owner is back at the machine,
   wait for them to say it again. The unit suite loads every core for minutes on a shared machine,
   and the browser suite also takes over the screen and the pointer. Run only the tests a change
   touches, by `--gtest_filter`, and ask before anything wider. Say so in the report when a full suite was skipped, and which tests
   did run.
...
| 5b A split as one sidebar entry | built, every hand row passed, perf measured — see docs/stage5b-findings.md, docs/superpowers/specs/2026-09-23-stage-5b-split-entry-design.md and docs/stage5b-hand-checks.md. Answers the 5a hand pass. Dragging a row now opens an 80px strip beside the page to drop on, because the page swallows drags on macOS (1e5d0ae). A split draws as one row with its halves side by side; two pinned entries sharing the screen are linked in the model (schema 7) and move, file, pin, unpin and relaunch as one entry; ending the split unlinks them and closing a tab does not (f9b06b6). **One older defect found and fixed**: a pinned tab came back cold after a relaunch, its page a second time in Today, because restore asked for the entry before the model file had loaded; the bind now waits for the load (666f923). All 800 unit tests and all 93 browser tests passed in whole-suite runs on 2026-09-24, with the owner's approval; the browser suite needed one retry, the split-comes-back-in-a-space-not-on-screen test running past the launcher's 45 s limit once under a load average near 400 and then passing in 6 s, three more times on its own. Two unit tests added after that run pass on their own. **All eight hand rows passed on 2026-09-23.** Both findings from the pass are closed: a pinned row can now be dropped at the bottom of Pinned (3fc6eb7), and the divider lag was partly the sidebar rebuilding on every step (791118f) and otherwise Chromium's own, lagging the same in plain Chromium. Added at the owner's request: dropping a row on the middle of another splits the two (73314f4), hand rows 9-11 passed; the strip beside the page no longer offers the page on screen a place beside itself (ea29ff9); and a split row has a handle between its halves that moves the split whole, while dragging either half away ends the split wherever it lands, Today and Pinned alike (aaa1c16), hand rows 12-18 passed on 2026-09-24; the handle's room now opens only under the pointer, sliding the halves apart and back (0b283e4), hand row 19 passed the same day. Perf 2026-09-24 (docs/perf/2026-09-24-stage5b.md): 1373 MB idle, startup 3617 ms under load 16, and 10 processes in two of three runs that two further launches did not reproduce (9 each). Both suites ran again after 0b283e4: all 93 browser tests passed, and 823 of 824 unit tests, the one left being `SidebarViewsTest.ClickingAnArchivedRowReopensItAndDropsIt`, which hung now and then on builds from before this work too. It is fixed (b0ecf1e): the hang was never at the pointer but in the drain before the click, where the test environment halts the thread pool while the compositor on the main thread waits for Dawn to compile a pipeline on that pool, so the fixture now drains the main thread alone; under the same load the old test hung 4 times in 100 and the new one passed 230 in a row |
| Welcome and Zen/Arc import | built and green 2026-09-25, the 14 hand rows in docs/welcome-hand-checks.md and perf still owed — see docs/welcome-findings.md, docs/superpowers/specs/2026-09-24-welcome-and-import-design.md, its mockups in docs/superpowers/specs/2026-09-24-welcome-mockups/, the plan docs/superpowers/plans/2026-09-24-welcome-and-import.md and the format reference docs/research/zen-arc-import-formats.md. A first-run card over the page area (import, spaces and logins, search engine, default browser, sync once it exists, tips), and an "Import from Zen or Arc" command. Pulls R6.5's spaces half forward. **Next after it, at the owner's request:** talk through importing from other popular browsers (Chrome, Firefox, Safari: bookmarks, history, passwords), then folder sync of spaces, pinned tabs, folders, favourites and site rules through a folder the owner already syncs (chosen over a sync server or Google's sync, which Chromium forks may not use) |
