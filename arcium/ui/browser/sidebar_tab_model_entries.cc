...
// SidebarTabModel's other half: the profile's persistent entries — the
// favourites, the pinned rows and the folders that hold them. How an entry
// finds and claims a tab, how one is drawn whether it is warm or cold, and
// every command that acts on one. An entry owns identity and outlives its
// tab, which is what makes it a different thing from the live tabs in
